...
GrowUndoCommand::GrowUndoCommand(VisualPhysicsActorList actors_to_grow, Room* room)
: room_(room),
  first_run_(true) {
  actors_to_grow_ = QList<VisualPhysicsActorId>();
  for_each(VisualPhysicsActor* actor, actors_to_grow) {
    actors_to_grow_.push_back(actor->unique_id());
    actors_size_was_changed_.insert(actor->unique_id(), true);
  }
}

GrowUndoCommand::~GrowUndoCommand() {
}

void GrowUndoCommand::undo() {
  if (!did_redo_change_something_) {
    UndoRedoStack::last_command_changed_something = false;
    return;
  }

  bool is_anything_shrunk = false;

  for_each(VisualPhysicsActorId actor_id, actors_to_grow_) {
    ...
    if (actor != NULL) {
      if (actors_size_was_changed_[actor_id]) {
        if (actor->children().empty()) {
          ...
          if (willGrowCommandChangeSomething(actor->children(), 1/VPA_GROW_FACTOR)) {
  ...
  UndoRedoStack::last_command_changed_something = is_anything_shrunk;
}

void GrowUndoCommand::redo() {
  ...
  for_each(VisualPhysicsActorId actor_id, actors_to_grow_) {
    ...
    if (actor != NULL) {
      if (actor->children().empty()) {
        ...
        if (willGrowCommandChangeSomething(actor->children(), VPA_GROW_FACTOR)) {
...
bool GrowUndoCommand::willGrowCommandChangeSomething(VisualPhysicsActorList actors, Ogre::Real scale_factor) {
  for_each(VisualPhysicsActor* actor, actors) {
    if (Ogre::Vector3(MAX_ACTOR_SIZE) < scale_factor*actor->scale() || Ogre::Vector3(MIN_ACTOR_SIZE) > scale_factor*actor->scale()) {
      return false;
    }
  }
  return true;
...
void GrowUndoCommand::updateLabelSize(VisualPhysicsActor* actor, Ogre::Real size_factor) {
  if (actor->label() != NULL) {
