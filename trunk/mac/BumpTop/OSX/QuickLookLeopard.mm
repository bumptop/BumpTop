...
#import "QuickLookLeopard.h"

#include "BumpTop/QStringHelpers.h"

// Private interface obtained from: http://www.cocoadev.com/index.pl?QuickLook
...
void LeopardQuickLookInterface::previewPathsInSharedPreviewPanel(QHash<QString, VisualPhysicsActorId> paths) {
  NSArray* path_array = NSURLArrayFromQStringListPaths(paths.keys());
  [[QLPreviewPanel sharedPreviewPanel] setURLs:path_array currentIndex:0 preservingDisplayState:YES];
  [[QLPreviewPanel sharedPreviewPanel] makeKeyAndOrderFrontWithEffect:1];   // 1 = fade in
}

bool LeopardQuickLookInterface::sharedPreviewPanelIsOpen() {
  return [[QLPreviewPanel sharedPreviewPanel] isOpen];
}

void LeopardQuickLookInterface::closeSharedPreviewPanel() {
  [[QLPreviewPanel sharedPreviewPanel] closePanel];
}

void LeopardQuickLookInterface::setSharedPreviewPanelPaths(QHash<QString, VisualPhysicsActorId> paths) {
  NSArray* path_array = NSURLArrayFromQStringListPaths(paths.keys());
  [[QLPreviewPanel sharedPreviewPanel] setURLs:path_array];
}

NSArray* LeopardQuickLookInterface::NSURLArrayFromQStringListPaths(QList<QString> paths) {
 NSMutableArray* path_array = [NSMutableArray arrayWithCapacity:paths.count()];
 for_each(QString path, paths) {
   [path_array addObject:[NSURL fileURLWithPath:NSStringFromQString(path)]];
 }
 return path_array;
}
