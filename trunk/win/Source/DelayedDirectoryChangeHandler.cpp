...
void CDirChangeNotification::PostonFileAdded(LPCTSTR szFileName)
{
	/*
	ASSERT(szFileName);
	m_eFunctionToDispatch	= eonFileAdded;
...
void CDirChangeNotification::PostonFileRemoved(LPCTSTR szFileName)
{
	/*
	ASSERT(szFileName);
	m_eFunctionToDispatch	= eonFileRemoved;
	m_szFileName1			= _tcsdup(szFileName) ;
	//
...
void CDirChangeNotification::PostonFileNameChanged(LPCTSTR szOldName, LPCTSTR szNewName)
{
	/*
	ASSERT(szOldName && szNewName);

	m_eFunctionToDispatch	= eonFileNameChanged;
...
void CDirChangeNotification::PostonFileModified(LPCTSTR szFileName)
{
	/*
	ASSERT(szFileName);

	m_eFunctionToDispatch	= eonFileModified;
	m_szFileName1			= _tcsdup(szFileName);
	//
	// post the message so it'll be dispatched by another thread.
	PostNotification();
...
void CDirChangeNotification::PostonReadDirectoryChangesError(DWORD dwError, LPCTSTR szDirectoryName)
{
	ASSERT(szDirectoryName);

	m_eFunctionToDispatch = eonReadDirectoryChangesError;
	m_dwError			  = dwError;
...
void CDirChangeNotification::PostonWatchStarted(DWORD dwError, LPCTSTR szDirectoryName)
{
	ASSERT(szDirectoryName);

	m_eFunctionToDispatch = eonWatchStarted;
	m_dwError			  =	dwError;
	m_szFileName1		  = _tcsdup(szDirectoryName);
...
void CDirChangeNotification::PostonWatchStopped(LPCTSTR szDirectoryName)
{
	ASSERT(szDirectoryName);

	m_eFunctionToDispatch = eonWatchStopped;
	m_szFileName1		  = _tcsdup(szDirectoryName);
...
//
long CDelayedNotificationWindow::s_RefCnt = 0L;
...
{
	long nRefCnt = -1;
	if( (nRefCnt = InterlockedDecrement(&s_RefCnt)) == 0 )
	{
...
//
long	CDelayedNotificationThread::s_RefCnt = 0L;
	...
	do{
		while(GetMessage(&msg, NULL, 0,0))//note GetMessage() can return -1, but only if i give it a bad HWND.(HWND for another thread for example)..i'm not giving an HWND, so no problemo here.
		{
			if(msg.message == UWM_DELAYED_DIRECTORY_NOTIFICATION)
			{
				...
				//			and the change handler object was deleted before the notification could be dispatched to this function.
				//
				//		* Somebody's implementation of an overridden VRTUAL function caused an exception
...
void CDelayedDirectoryChangeHandler::onFileAdded(LPCTSTR strFileName)
{
	m_pRealHandler->onFileAdded(strFileName);
}

void CDelayedDirectoryChangeHandler::onFileRemoved(LPCTSTR strFileName)
{
	m_pRealHandler->onFileRemoved(strFileName);
}

void CDelayedDirectoryChangeHandler::onFileModified(LPCTSTR strFileName)
{
	m_pRealHandler->onFileModified(strFileName);
}

void CDelayedDirectoryChangeHandler::onFileNameChanged(LPCTSTR strOldFileName, LPCTSTR strNewFileName)
{
	m_pRealHandler->onFileNameChanged(strOldFileName, strNewFileName);
}

void CDelayedDirectoryChangeHandler::onReadDirectoryChangesError(DWORD dwError, const CString & strDirectoryName)
{
	CDirChangeNotification * p = GetNotificationObject();
	ASSERT(p);
	if(p) p->PostonReadDirectoryChangesError(dwError, strDirectoryName);
}
...
void CDelayedDirectoryChangeHandler::onWatchStarted(DWORD dwError, const CString & strDirectoryName)
{
	if(!(m_dwFilterFlags & CDirectoryChangeWatcher::FILTERS_NO_WATCHSTART_NOTIFICATION))
	{
		...
		if(p) p->PostonWatchStarted(dwError, strDirectoryName);
	}
}
...
void CDelayedDirectoryChangeHandler::onWatchStopped(const CString & strDirectoryName)
{
	if(!(m_dwFilterFlags & CDirectoryChangeWatcher::FILTERS_NO_WATCHSTOP_NOTIFICATION))
	{
		...
		if(p){
			...
			p->PostonWatchStopped(strDirectoryName);
...
//
{
	if(	eEvent == CDirChangeNotification::eonWatchStarted 
	||	eEvent == CDirChangeNotification::eonWatchStopped
	||	eEvent == CDirChangeNotification::eonReadDirectoryChangesError)
	{
		return true;
	}
	else
...
//					CDirectoryChangeHandler::onFilterNotification()
//
//	RETURN VALUE:
//	If this function returns true, the notification function is called.
//	If it returns false, the notification is ignored.
//			The client is notified by calling CDirectoryChangeHandler's VRTUAL functions onFileAdded(),onFileRemoved(), etc.
{
	ASSERT(pNot);
	ASSERT(m_pRealHandler);
	...
	//
	//	See if user wants to test CDirectoryChangeHandler::onFilterNotification()
	//	before tests are performed against the file name, and filter specifications
	//
	if((m_dwFilterFlags & CDirectoryChangeWatcher::FILTERS_TEST_HANDLER_FIRST)//specified that CDirectoryChangeHandler::onFilterNotification is to be called before any other filter tests
	&&	!(m_dwFilterFlags & CDirectoryChangeWatcher::FILTERS_DONT_USE_HANDLER_FILTER)//and did not specify that this CDirectoryChangeHandler::onFilterNotification is not to be called..
	...
	if(!(m_dwFilterFlags & CDirectoryChangeWatcher::FILTERS_DONT_USE_FILTERS))
	{
		...
		{
			if(pNot->m_eFunctionToDispatch != CDirChangeNotification::eonFileNameChanged)
				return false;
			else{
	...
	{
		if(m_pRealHandler->onFilterNotification(pNot->m_eFunctionToDispatch,
											  pNot->m_szFileName1,
											  pNot->m_szFileName2))
		{
			return true;
...
//			   notifiy the client by calling the appropriate CDirectoryChangeHandler::onFilexxx() function.
		...
		if(NotifyClientOfFileChange(pNotification))
		{
			...
			case CDirChangeNotification::eonWatchStopped:
				...
				try{
					...
					MessageBeep(0xffff);
			#ifdef DEBUG
					MessageBox(NULL,_T("An RTFM Exception was raised in onWatchStopped() -- see Comments for CDirectoryChangeHandler::onWatchStopped() in DirectoryChanges.h."), _T("Programmer Note(DEBUG INFO):"), MB_ICONEXCLAMATION | MB_OK);
			#endif
				}
				//
				//	Signal that the onWatchStopped() function has been dispatched.
				//
				if(m_hWatchStoppedDispatchedEvent)
...
//
//	When shutting down, m_pRealHandler->onWatchStopped() will be called.
...
//
{
	...
	{
		TRACE(_T("WARNING: Unable to wait for notification that the onWatchStopped function has been dispatched to another thread.\n"));
...
const std::basic_string<TCHAR> & CDelayedDirectoryChangeHandler::GetChangedDirectoryName() const
{
	if(m_pRealHandler)
