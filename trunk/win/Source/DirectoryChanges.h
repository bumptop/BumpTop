...
// from a DLL simpler. All files within this DLL are compiled with the DIRECTORYCHANGES_EXPORTS
// symbol defined on the command line. This symbol should not be defined on any project
// that uses this DLL. This way any other project whose source files include this file see 
// DIRECTORYCHANGES_API functions as being imported from a DLL, whereas this DLL sees symbols
// defined with this macro as being exported.
#ifdef DIRECTORYCHANGES_EXPORTS
#define DIRECTORYCHANGES_API __declspec(dllexport)
#else
#define DIRECTORYCHANGES_API __declspec(dllimport)
#pragma comment(lib, "DirectoryChanges.lib")
#endif
...
class DIRECTORYCHANGES_API DirectoryChangeWatcher // CDirectoryChangeWatcher wrapper for BT_FileSystemManager
{
...
public:
	...
	DWORD WatchDirectory(const TCHAR * strDirToWatch, 
	...
	BOOL UnwatchDirectory(const TCHAR * strDirToStopWatching);
};

class CCriticalSection;

class DIRECTORYCHANGES_API CDirectoryChangeHandler
/***********************************
 A class to handle changes to files in a directory.  
 The VRTUAL onFilexxx() functions are called whenever changes are made to a watched directory that is being handled by this object...
 The onFilexxx() functions execute in the context of the main thread if true is passed to the constructor of CDirectoryChangeWatcher,
 OR they fire in the context of a worker thread if false is passed to the constructor of CDirectoryChangeWatcher
  ...
  NOTES: 
   ...
   these changes all map out to onFileModified() which doesn't specify the type of modification.


  NOTE:   The CDirectoryChangeHandler::onFilexxx() functions
...
public:
	...
	const std::basic_string<TCHAR> & GetChangedDirectoryName() const { return m_strChangedDirectoryName;}//WARNING: don't use this, this function will be removed in a future release.
...
protected:
	...
	virtual void onFileAdded(LPCTSTR strFileName); //=0;
			//
			//	onFileAdded()
			//
			...
			//
	virtual void onFileRemoved(LPCTSTR strFileName);// = 0;
			//
			//	onFileRemoved()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void onFileNameChanged(LPCTSTR strOldFileName, LPCTSTR strNewFileName);// = 0;
			//
			//	onFileNameChanged()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void onFileModified(LPCTSTR strFileName);// = 0;
			//
			//	onFileModified()
			//
	...
	virtual void onReadDirectoryChangesError(DWORD dwError, const TCHAR * strDirectoryName);
			//
			...
			//
			//	onWatchStopped() will not be called.


	virtual void onWatchStarted(DWORD dwError, const TCHAR * strDirectoryName);
			//
			//	void onWatchStarted()
			//
	...
	virtual void onWatchStopped(const TCHAR * strDirectoryName);
			//
			//	void onWatchStopped()
			//
	...
	void SetChangedDirectoryName(const TCHAR * strChangedDirName);//please don't use this function, it will be removed in future releases.
	
private:
	long m_nRefCnt;
	
	std::basic_string<TCHAR> m_strChangedDirectoryName;//will be removed in a future release.
...
///////////////////////////////////////////////////////////
#ifdef DIRECTORYCHANGES_EXPORTS // ATLMFC dependent code is built separately
class CDirectoryChangeWatcher  
/***************************************
	...
	Notifications (calling CDirectoryChangeHandler's VRTUAL functions) are executed 
...
public:
	...
	enum	{  //options for determining the behavior of the filter tests.
			   //
			   FILTERS_DONT_USE_FILTERS		= 1, //never test the include/exclude filters. CDirectoryChangeHandler::onFilterNotification() is still called.
			   ...
			   FILTERS_TEST_HANDLER_FIRST	= 16, //test CDirectoryChangeHandler::onFilterNotification() before checking the include/exclude filters. the default is to check the include/exclude filters first.
			   FILTERS_DONT_USE_HANDLER_FILTER = 32, //CDirectoryChangeHander::onFilterNotification() won't be called.
			   FILTERS_NO_WATCHSTART_NOTIFICATION = 64,//CDirectoryChangeHander::onWatchStarted() won't be called.
			   FILTERS_NO_WATCHSTOP_NOTIFICATION  = 128,//CDirectoryChangeHander::onWatchStopped() won't be called.
