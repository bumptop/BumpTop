        ...
        if (isalpha(yyCh) || yyCh == '_') {
            ...
            case 'Q':
                ...
                if (yyIdent == QLatin1String("QT_TRANSLATE_NOOP3_UTF8"))
                    return Tok_translateUtf8;
				if (yyIdent == QLatin1String("QT_NT"))
					return Tok_notranslate;
				if (yyIdent == QLatin1String("QString_NT"))
					return Tok_notranslate;
                break;
            case 'T':
...
static QString remainingFileString; //used for getting source lines
static int lastLineNumber;
FILE * warnStringsFile = NULL;
bool outputWarnings = false;
unsigned int stringCount = 0;
    
static void WarnUnwrappedStringLitteral()
{
	static const QChar chNewline(QLatin1Char('\n'));
	static const QString strEquals(QLatin1String("==")), strNotEquals(QLatin1String("!="));
	static const QString strInsert(QLatin1String("insert(\""));
	static const QString immediateLeftToExclude [] =
	{
		QLatin1String("insert(\""), QLatin1String("[\""), QLatin1String("/ \"") /*path join*/, QLatin1String("QT_NT(L\"")/*matchString doesn't match wide string*/,
		QLatin1String("Message(\""), QLatin1String("printTimedUnique(\""), QLatin1String("printUniqueError(\""), QLatin1String("printUnique(\""),
		
	};

	static const QString containsToExclude [] =
	{
		QLatin1String("QT_TR_NOO"), QLatin1String("QT_TRANSLATE_NOOP"), //it's assumed that if the line contains these then other strings shouldn't be translated
		QLatin1String("puts"), QLatin1String("printf"), QLatin1String("wprintf"), QLatin1String("consoleWrite"), 
		QLatin1String("LOG("), QLatin1String("LOG_START("), QLatin1String("LOG_FINISH("),
		QLatin1String("throw ")/*some sentences might have throw*/,
		QLatin1String("TEST_LOG("), QLatin1String(" << "),
		QLatin1String("dismiss(\""), QLatin1String("make_file"),
		QLatin1String("contains(\""), QLatin1String("value(\""), QLatin1String("getValue"), QLatin1String("getString"), QLatin1String("isMember(\""),
		QLatin1String("setTextureID"), QLatin1String("texMgr->"), QLatin1String("profiler->"),
	};

	/*QString lineText = yyInStr;
	for (int i = 1; i < yyLineNo; i++)
		lineText = lineText.mid(lineText.indexOf(QLatin1String("\n")) + 1);
	lineText = lineText.mid(0, lineText.indexOf(QLatin1String("\n")));*/
	//QString lineText = QLatin1String("");
	for (int i = lastLineNumber; i < yyLineNo; i++)
		remainingFileString = remainingFileString.mid(remainingFileString.indexOf(chNewline) + 1);
	QString lineText = remainingFileString.mid(0, remainingFileString.indexOf(chNewline));;
	lastLineNumber = yyLineNo;

	for (unsigned int i = 0; i < _countof(containsToExclude); i++)
	{
		if (lineText.contains(containsToExclude[i]))
			return;
	}
	...
	int stringIndex = lineText.indexOf(yyString);
	int strEqualsIndex = lineText.indexOf(strEquals);
	int strNotEqualsIndex = lineText.indexOf(strNotEquals);

	if (strEqualsIndex > -1 || strNotEqualsIndex > -1)
		return; //TODO: check if string is actually the comparison operand

	//some immdiate left occurs more than once per line, so second fails
	for (unsigned int i = 0; i < _countof(immediateLeftToExclude); i++)
	{
		int index = lineText.indexOf(immediateLeftToExclude[i]);
		if (index > -1)
			if (index + immediateLeftToExclude[i].size() == stringIndex)
				return;
	}
	...
	stringCount++;
	if (outputWarnings)
		printf("%s(%d) : warning unwrapped string: %s \n", qPrintable(yyFileName), yyLineNo, qPrintable(lineText));
	if (warnStringsFile)
		fprintf(warnStringsFile, "%s : %s \n", qPrintable(yyFileName), qPrintable(lineText));
}
static void parse(Translator *tor, const QString &initialContext, const QString &defaultContext)
{
	...
	yyTok = getToken();
	Token lastToken = (Token)yyTok;
    while (yyTok != Tok_Eof) {
		//qDebug() << "TOKEN: " << yyTok;
		switch (yyTok) {
		case Tok_notranslate :
			yyTok = getToken();
            match(Tok_LeftParen);
			matchString(&text);
        ...
        case Tok_Arrow:
            ...
            if (yyTok == Tok_tr || yyTok == Tok_trUtf8)
				printf("%s(%d) : error : Cannot invoke tr() like this \n", qPrintable(yyFileName), yyLineNo);
            break;
        case Tok_ColonColon:
    ...
    if (yyBraceDepth != 0)
		printf("%s(%d) : warning : Unbalanced braces in C++ code (or abuse of the C++ preprocessor) \n",
                  qPrintable(yyFileName), yyBraceLineNo);
    else if (yyParenDepth != 0)
		printf("%s(%d) : warning : Unbalanced parentheses in C++ code (or abuse of the C++ preprocessor) \n",
                 qPrintable(yyFileName), yyParenLineNo);

	/*if (stringCount)
		printf("%s : warning: unwrapped stringCount=%d \n", (const char *)qPrintable(yyFileName), stringCount);
	else
		printf("%s : message: no unwrapped strings \n", (const char *)qPrintable(yyFileName));*/
	//qWarning("%s:%d: stringCount %d", qPrintable(yyFileName), yyLineNo, stringCount);
}
...
bool loadCPP(Translator &translator, QIODevice &dev, ConversionData &cd)
{
	...
	yyFileName = QFileInfo(yyFileName).fileName();
	yySavedBraceDepth.clear();
    yySavedParenDepth.clear();
    yyBraceDepth = 0;
    yyParenDepth = 0;
