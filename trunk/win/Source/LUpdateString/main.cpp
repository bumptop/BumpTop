...
extern bool outputWarnings;
extern FILE * warnStringsFile;
...
static void printUsage()
{
    printOut(QObject::tr(
        ...
        "Options:\n"
		"    -log <filepath>\n"
		"           Outputs unwrapped strings to file."
...
int main(int argc, char **argv)
{
	...
	QString warnStringsFilePath = QLatin1String("LUpdateStrings_warning.txt");
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == QLatin1String("-help")
            ...
            numFiles++;
            continue;
		} else if (arg == QLatin1String("-outputWarnings")){
			outputWarnings = true;
			continue;
		} else if (arg == QLatin1String("-log")) {
            ++i;
            if (i == argc) {
                qWarning("The -log option should be followed by a file path.");
                return 1;
            }
            warnStringsFile = fopen(args[i].toAscii(), "wt");
            continue;
	...
	//warnStringsFile = fopen("LUpdateStrings_warn.txt", "wt");
	...
	if (warnStringsFile)
		fclose(warnStringsFile);
	
	if (stringCount)
		printf("LUpdateString\\unwrapped_strings.txt(1) : warning : total unwrapped strings = %u \n", stringCount);
	else
		puts("LUpdateString\\unwrapped_strings.txt(1) : info : found no unwrapped strings");
