...
#ifndef BT_CHOOSE_VERSION_DIALOG
#define BT_CHOOSE_VERSION_DIALOG
...
class ChooseVersionDialog : public Win32Dialog
{
	Q_DECLARE_TR_FUNCTIONS(ChooseVersionDialog)

private:
	QList<HoverControl*> _hoverButtons;
	static WNDPROC prevWndProc;
	static const int MAX_CHARS;
	HWND _dlgHwnd;
	HBITMAP _backgroundBmp;
	bool _useExpiredBackground;
	
public:
	ChooseVersionDialog();
	virtual ~ChooseVersionDialog();
	...
	static LRESULT dialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

	// reset
	virtual void resetToDefault();

	HoverControl* getHoverControl(HWND handle);
};

#endif // BT_CHOOSE_VERSION_DIALOG